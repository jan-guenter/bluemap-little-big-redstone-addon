...
observed defects until the owner explicitly accepts the staged result. Record
the exact staged artifact identity. If instrumentation or an overlay changes
the staged JAR, do not treat that acceptance as proof that the production JAR
ran.

## Acceptance and release

Freeze the functional entries of the artifact that will be released. The
writer refuses to overwrite an existing acceptance record:

```bash
bluemap-addon-toolkit jar-entries write \
  --jar /absolute/path/accepted-production.jar \
...
Record `visual_acceptance: true` under `owner_accepted_staging`, including the
exact artifact the owner reviewed. If that artifact differs from production,
record the difference and a separate sealed exact-production runtime PASS.
Record the production JAR, sources JAR, POM and Gradle module file names, sizes
and hashes under `final_release_artifacts`.
...
Merge only after final-version CI passes this gate and any staged-to-production
artifact gap has a sealed exact-production runtime PASS. Create an annotated
